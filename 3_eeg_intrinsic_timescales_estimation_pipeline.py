    ...
    for g in group:
        ...
        if estimate_timescales:
            for t in timepoints:
                for control in control_condition:
                    print(f'########### RUNNING MODEL FOR {t} - {control}')
                    ...
                    tau_estimate.estimate_eeg_tau(sub_list,t,half_sample,datapth,channels,nlags,removelag0,resultspth,group_flag,control)
            
        if ACFplot:
            for t in timepoints:
                for control in control_condition:
                    print(f'########### RUNNING ACFplots FOR {t} - {control}')
                    datapth = f'/eeg/ITS_Project_NewPreproc/{t}/10s/'
                    resultspth = f'/eeg/EEG_timescales/Results/{t}/10s'
                    if not os.path.isdir(resultspth):
                        os.mkdir(resultspth)
                    sub_list = tau_estimate.get_subj_list(datapth,t)
                    tau_estimate.make_ACF_plot(sub_list, t, half_sample, datapth, channels, nlags, removelag0, resultspth, group_flag, control)
        ...
        for t in timepoints:
            for control in control_condition:
                print(f'{g} - {t}')
                resultspth = f'/eeg/EEG_timescales/Results/{t}/10s'
                tau = pd.read_csv(os.path.join(resultspth,f'EEGdata_tau_{t}_10s_numpy{nlags}_nooutliers{group_flag}{control}.csv'))
                ...
                tau_nohighnan.to_csv(os.path.join(resultspth,f'EEGdata_tau_{t}_10s_numpy{nlags}_nooutliers_nohighnan{group_flag}{control}.csv'))
                ...
                tau_nohighnan_long_data.to_csv(f'/eeg/EEG_timescales/Results/{t}/10s/EEGdata_tau_{t}_10s_numpy1000_longformat_seconds_nooutliers_lowmovement_nohighnans{group_flag}{control}.csv')

                # summarize epochs with too many nans
                samplesize_filename = f'/eeg/EEG_timescales/Results/samplesize_10s_numpy{nlags}_nooutliers{group_flag}{control}.txt'
                ...
                #plt.legend(areas)
                print(areas)
                plt.suptitle(f'Nan percentage per electrode in {t} - {g} - {control} \n Total nan % = {nan_total_perc}, Max % = {np.max(nan_per_electrode)}')
                plt.savefig(f'/eeg/EEG_timescales/Figure/nan_count_{t}_{group_flag}{flag}{control}.png')
                plt.savefig(f'/eeg/EEG_timescales/Figure/nan_count_{t}_{group_flag}{flag}{control}.pdf')
                plt.close()

                #r2_values = np.loadtxt(os.path.join(resultspth,f'EEGdata_{t}_10s_R2_{group_flag}{control}.txt'))
                #if np.isinf(r2_values).any():
                #    print(f"Warning: 'r2_values' contains inf values for {t}, {group_flag}, {control}!")
                #    r2_values = np.where(np.isinf(r2_values), np.nan, r2_values)

                #with open(os.path.join(resultspth,f'EEGdata_{t}_R2averageOfExponentialFit_{group_flag}{control}.txt'), 'w') as f:
                    #f.write(f'The average R2 value is {np.nanmean(r2_values)}\n')
                    #f.write(f'Number of nans =  {np.isnan(r2_values).sum()}\n')
                    #f.write(f'% of nans =  {((np.isnan(r2_values).sum()) / r2_values.size) *100}\n')
                    #f.close()

print('\n \n \n ########    ALL DONEEEEEE!!!!!!!!!!!!!')
        ## THE TAU DATASETS ARE THEN PASSED ON TO MATLAB TO ESTIMATE NANS INTERPOLATING FROM NEARBY ELECTRODES
